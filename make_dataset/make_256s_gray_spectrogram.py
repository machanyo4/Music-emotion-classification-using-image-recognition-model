...
for q in ['Q1', 'Q2', 'Q3', 'Q4']:
    os.makedirs(f"{grayscale_spec_dataset_path}/{q}/256s", exist_ok=True)
...
for q in ['Q1', 'Q2', 'Q3', 'Q4']:
    ...
    for file in filelist:
        ...
        raw_path = f"{grayscale_spec_dataset_path}/{q}/256s/{filename_no_extension}"
        for window_value in window_values:
            ...
            if not os.path.exists(save_name):
                ...
                print(f"[LOG] {filename} -> /spec/grayscale/{q}/256s/{filename_no_extension}{param_data}.png")
    print("--------------------")
...
# 各ディレクトリのデータ数を表示
for q in ['Q1', 'Q2', 'Q3', 'Q4']:
    count = count_file(f"{grayscale_spec_dataset_path}/{q}/256s")
    print(f"[INFO] Datas in spec/grayscale/{q}/256s: {count}")
