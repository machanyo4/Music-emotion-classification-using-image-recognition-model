...
if not (os.path.exists(dataset_path + "/spec/Q1/2048s")):
    os.mkdir(dataset_path + "/spec/Q1/2048s")
if not (os.path.exists(dataset_path + "/spec/Q2/2048s")):
    os.mkdir(dataset_path + "/spec/Q2/2048s")
if not (os.path.exists(dataset_path + "/spec/Q3/2048s")):
    os.mkdir(dataset_path + "/spec/Q3/2048s")
if not (os.path.exists(dataset_path + "/spec/Q4/2048s")):
    os.mkdir(dataset_path + "/spec/Q4/2048s")
...
sampling = 22050
window_values = [2044, 2046, 2048, 2050, 2052]
overlap_rate = 0.5
...
for file in Q1_filelist:
    ...
    raw_path = spec_dataset_path + '/Q1/2048s/' + filename_no_extension
    for window_value in window_values:
        ...
        if not os.path.exists(save_name):
            ...
            print("[LOG] " + filename + " -> /spec/Q1/2048s/" +  filename_no_extension + param_data + ".png")
print("--------------------")
...
for file in Q2_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q2/2048s/' + filename_no_extension
    for window_value in window_values:
        # パラメータ設定
        window = window_value
        overlap = int(window*overlap_rate)
        ...
        if not os.path.exists(save_name):
            save_spectrogram(save_name, db, signal)
            print("[LOG] " + filename + " -> /spec/Q2/2048s/" +  filename_no_extension + param_data + ".png")
print("--------------------")
...
for file in Q3_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q3/2048s/' + filename_no_extension
    for window_value in window_values:
        # パラメータ設定
        window = window_value
        overlap = int(window*overlap_rate)
        ...
        if not os.path.exists(save_name):
            save_spectrogram(save_name, db, signal)
            print("[LOG] " + filename + " -> /spec/Q3/2048s/" +  filename_no_extension + param_data + ".png")
print("--------------------")
...
for file in Q4_filelist:
    filename = os.path.basename(file)                       # filename を格納
    filename_no_extension = os.path.splitext(filename)[0]   # extention を排除して格納
    raw_path = spec_dataset_path + '/Q4/2048s/' + filename_no_extension
    for window_value in window_values:
        # パラメータ設定
        window = window_value
        overlap = int(window*overlap_rate)
        ...
        if not os.path.exists(save_name):
            save_spectrogram(save_name, db, signal)
            print("[LOG] " + filename + " -> /spec/Q4/2048s/" +  filename_no_extension + param_data + ".png")
print("--------------------")
...
# How many datas?    
print("[INFO] Datas in spec/Q1/2048s: ", end='')
print(count_file(spec_dataset_path + "/Q1/2048s"))
print("[INFO] Datas in spec/Q2/2048s: ", end='')
print(count_file(spec_dataset_path + "/Q2/2048s"))
print("[INFO] Datas in spec/Q3/2048s: ", end='')
print(count_file(spec_dataset_path + "/Q3/2048s"))
print("[INFO] Datas in spec/Q4/2048s: ", end='')
print(count_file(spec_dataset_path + "/Q4/2048s"))