...
for class_idx, img_tensors in correct_images.items():
    for idx, img_tensor in enumerate(img_tensors):
        ...
        # ヒートマップを保存（カラーバーと余白なし）
        fig, ax = plt.subplots(figsize=(5, 5))
        ax.imshow(heatmap, cmap='jet')
        ax.axis('off')  # 軸を非表示
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # 余白を完全になくす
        plt.savefig(f'../result/heatmap/heatmap_class_{class_names[class_idx]}_pred_{class_names[class_idx]}_{idx}.png', 
...
for class_idx, img_tensors in incorrect_images.items():
    for idx, img_tensor in enumerate(img_tensors):
        ...
        ax.axis('off')  # 軸を非表示
        plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # 余白を完全になくす
        plt.savefig(f'../result/heatmap/heatmap_class_{class_names[class_idx]}_pred_{class_names[pred_label]}_{idx}.png', 
                    bbox_inches='tight', pad_inches=0)
        plt.close()

print("[LOG] Heatmaps saved successfully without colorbar or margins.")